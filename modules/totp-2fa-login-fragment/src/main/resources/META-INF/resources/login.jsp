...
<c:choose>
	...
	<c:otherwise>
		...
		<div class="login-container">
			...
			<!-- MW: Added data-senna-off="true" to the form to turn Senna SPA off for this form as it interferes with the parameterMap in Authenticators. -->
			<aui:form action="<%= loginURL %>" data-senna-off="true" autocomplete='<%= PropsValues.COMPANY_SECURITY_LOGIN_FORM_AUTOCOMPLETE ? "on" : "off" %>' cssClass="sign-in-form" method="post" name="<%= formName %>" onSubmit="event.preventDefault();" validateOnBlur="<%= false %>">
				...
				<aui:fieldset>
					...
					<!-- MW: Custom dynamic-include added so we can inject in the Authenticator Code field after Password field. -->
					<liferay-util:dynamic-include key="com.liferay.login.web#/login.jsp#loginFieldsPost" />
